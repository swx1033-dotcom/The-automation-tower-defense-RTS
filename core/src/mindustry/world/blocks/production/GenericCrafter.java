...
public class GenericCrafter extends Block{
    ...
    public class GenericCrafterBuild extends Building{
        ...
        public boolean checkInputMaterials(){
            if(consumes == null) return true;

            for(var consume : consumes.items){
                if(consume != null && items.get(consume.item) < consume.amount){
                    return false;
                }
            }
            return true;
        ...
        public boolean checkOutputSpace(){
            if(outputItems != null){
                for(var output : outputItems){
                    if(items.get(output.item) + output.amount > itemCapacity){
                        return false;
        ...
        public void updateTile(){
            if(efficiency > 0){
                if(!paused){
                    if(!checkInputMaterials() || !checkOutputSpace()){
                        paused = true;
                    }
                }else{
                    if(checkInputMaterials() && checkOutputSpace()){
